...
class ModelProductProduct extends Model {
    public function addProduct($id, $data,$image) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "product_sac SET customer_id = '" . (int)$id . "', size = '" . $this->db->escape($data['size'])."', text ='". $this->db->escape($data['text'])."', kind ='".$this->db->escape($data['kind']). "', image = '" . $this->db->escape($data['image']) . "', price = '" . $this->db->escape($data['price']) . "',address= ' ', status= '0', date_added = NOW()");
